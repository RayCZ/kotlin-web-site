...
Secondary Constructors ：次要建構元，使用 `constructor`
...
Creating instances of classes ：創建類別的實例，類別名稱 + ()

To create an instance of a class, we call the constructor as if it were a regular function:

為了創建一個類別的實例，如果建構元是一個常規函數，我們可以調用建構元：
``` kotlin
val invoice = Invoice()
...
創建內嵌、內部、匿名內部類別的實例描述在[內嵌類別](nested-classes.md)。
...
在 Kotlin 的所有類別有一個共同的超 (父) 類別 `Any` ，對於一個類別沒有宣告超 (父) 類型， `Any` 類別是預設超 (父) 類別：
``` kotlin
...
去宣告明顯的超 (父) 類型，在類別標頭中我們在冒號之後放置類型：
``` kotlin
...
>
> 在類別中 `open` 註釋與 Java 的 `final` 相反：它允許從這個類別讓其他類別去繼承。根據預設，在 Kotlin 所有類別是 `final` 對應於 [Effective Java, 3rd Edition](http://www.oracle.com/technetwork/java/effectivejava-136174.html) , Item 19: *Design and document for inheritance or else prohibit it* 。
>
...
如果衍生 (子) 類別有主建構元，可以 (且必須) 使用主建構元的參數，在那裡初始化基礎類別。
...
如果類別沒有主建構元，接著每個次要建構元必須使用 `super` 關鍵字初始化基礎 (父) 類型，或調用 (委派) 其他建構元做該件事。注意：在這個情況，不同的次要建構元可以各自調用不同的基礎 (父) 類型的建構元：
...
Overriding Methods ：覆寫方法，使用 `open` 和 `override` 註釋
...
如我們之前所述，我們堅持在 Kotlin 中明確表達做的事情。並且不像 Java ， Kotlin 對於可覆寫的成員 (我們稱它們 `open`) 和對於覆寫 (衍生 (子) 類別註釋 `override` ) 需要明確註釋：
...
`Derived.v()` 需要修飾符 `override` - `override fun v() { ... }` 。如果 `override` 被遺忘，編譯器可能會抱怨。如果在函數中沒有 `open` 註釋，像 `Base.nv()` ，在子類別中使用相同簽名宣告一個方法是非法的，無論是使用 `override` 或不使用它。當在 `final` 類別 (即是類別沒使用 `open` 修飾符) 添加成員時， `open` 修飾符不影響。
...
`Derived.v()` 需要註釋 `override` - `override fun v() { ... }` 。如果 `override` 被遺忘，編譯器可能會抱怨。如果在函數中沒有 `open` 註釋，像 `Base.nv()` ，在子類別中使用相同簽名宣告一個方法是非法的，無論是使用 `override` 或不使用它。在一個 `final` 類別中 (例如：沒有 `open` 註釋的類別) ，公開的成員被禁用。

---

A member marked *override* is itself open, i.e. it may be overridden in subclasses. If you want to prohibit re-overriding, use *final*:

一個成員被標記 `override` 代表它本身是 `open` ，即是，在子類別中可以覆寫它。如果你想要禁用 `re-override` ，使用 `final` ：
``` kotlin
...
覆寫屬性的工作方式與覆寫方法類似；在超 (父) 類別宣告屬性，接著在衍生 (子) 類別重新宣告屬性，必須以 `override` 為宣告的開始，並且它們必須有兼容類型。透過衍生 (子) 類別初始化區塊或 getter 方法覆寫每個已宣告的屬性。
...
你也可以使用 `var` 屬性覆寫一個 `val` 屬性，但不可以反過來。這是允許的，因為一個 `val` 屬性本質上已宣告一個 getter 方法，並且在衍生 (子) 類別覆寫屬性為 `var` ，額外宣告一個 setter 方法。

Note that you can use the *override* keyword as part of the property declaration in a primary constructor.

注意：在主建構元中，你可以使用 `override` 關鍵字為屬性宣告的一部分。
``` kotlin 
interface Foo {
    val count: Int
}
...
在衍生 (子) 類別創建實例的建構期間，基礎 (父) 類別初始化為第一步完成 (只在基礎 (父) 類別建構元的參數執行之前) ，因此在衍生 (子) 類別的初始化邏輯運行之前發生。

```kotlin
//sampleStart
open class Base(val name: String) {

    init { println("Initializing Base") } // step 3

    open val size: Int = 
        name.length.also { println("Initializing size in Base: $it") } // step 4
}

class Derived(
    name: String,
    val lastName: String
) : Base(name.capitalize().also { println("Argument for Base: $it") }) { // step 2

    init { println("Initializing Derived") } // step 5
...
fun main() {
    println("Constructing Derived(\"hello\", \"world\")") // step 1
...
Calling the superclass implementation ：調用超 (父) 類別實作，使用 super 調用屬性或函數

Code in a derived class can call its superclass functions and property accessors implementations using the *super* keyword:

在衍生 (子) 類別中的代碼可以使用 `super` 調用它的超 (父) 類別函數或屬性存取器 (setter/getter) 實作：
...
在 Kotlin 中，透過以下規則規範實作繼承：如果一個類別從它的直屬超 (父) 類別，繼承很多相同成員的實作，它必須覆寫這些成員，並且提供它擁有的實作 (可能，使用其中一個繼承的實作) 。為了表示帶入繼承實作的超 (父) 類型，我們在尖括號中透過超 (父) 類型名稱使用 `super` 修飾，例如： `super<Base>` ：
...
繼承 `A` 和 `B` 都沒關係，並且使用函數 `a()` 和 `b()` 都沒有問題，因為 `C` 只繼承這些函數的每一個實作。但對於 `f()` 我們由 `C` 繼承兩個實作，因為我們在 `C` 必需覆寫 `f()` ，並且提供我們擁有的實作消除分歧。
...
一個類別和它的一些成員可以被宣告為  `abstract` 。一個抽象成員在它的類別本身不會有實作。注意：我們不需要使用 `open` 來註釋抽象類別或函數 - 不可而喻 (抽象代表已經是要去實作了) 。
...
如果你需要寫一個可以沒有類別實例被調用的函數，但需要存取類別的內部 (例如：工廠方法) ，你可以在該類別內編寫它為一個[物件宣告](object-declarations.md)的成員。
...
更具體來說，如果在你的類別內宣告一個[夥伴物件](object-declarations.md#companion-objects)，你將能調用類別內的成員，你將能使用與在 Java/C# 中調用靜態方法相同的語法來調用它的成員，只使用類別名稱為修飾符。